...
\begin{enumerate}[label=\textbf{RQ\arabic*.}]
    \item How do classical ML, deep learning, and transformer models compare on English suicidality and depression detection, and how much do results depend on dataset characteristics such as size, text length, and annotation quality?
    \item Can multilingual transformers (mBERT, XLM-RoBERTa) effectively detect depression in Russian-language social media when fine-tuned on Russian data?
    \item How well does zero-shot cross-lingual transfer work when a model is trained only on English and evaluated on Russian — and how many Russian-language examples are needed to close the performance gap?
    \item What do explainability methods (LIME, SHAP, and attention visualisation) reveal about what models actually learn, and what artifacts do they expose that evaluation metrics alone cannot detect?
\end{enumerate}

RQ1 covers the English-language benchmark (Chapters~\ref{ch:ml}--\ref{ch:bert}). RQ2 and RQ3 address the multilingual contribution — fine-tuned performance and zero-shot cross-lingual transfer respectively — both covered in Chapter~\ref{ch:multilingual}. RQ4 covers the explainability analysis in Chapter~\ref{ch:xai}.
